...
export class LargeSquareComponent implements OnInit {
  ...
  colorThisSide(currHeight: number, currWidth: number, isHorizontalMoving: boolean, isPositiveIncrement: boolean, connectedHeight: number, connectedWidth: number): elementPlacementInterface {
    let first = true;
    let lastSquare: elementPlacementInterface;
    if (currHeight < 0 || currHeight > LargeSquareComponent.TOTAL_HEIGHT - 1 || currWidth < 0 || currWidth > LargeSquareComponent.TOTAL_WIDTH - 1) {
      lastSquare = this.uncolorThisSide(isHorizontalMoving, isPositiveIncrement, connectedHeight, connectedWidth);
    } else {
      while (this.squareElements[currHeight] && this.squareElements[currHeight][currWidth] && (this.squareElements[connectedHeight][connectedWidth].parentClick || (this.origin.height === connectedHeight && this.origin.width === connectedWidth || first))) {
   ...
   * @param connectedWidth - The column for the connected tile to check if it is colored
   */
  uncolorThisSide(isHorizontalMoving: boolean, isPositiveIncrement: boolean, connectedHeight: number, connectedWidth: number): elementPlacementInterface {
    let nextBlock: number;
    while (this.squareElements[connectedHeight] && this.squareElements[connectedHeight][connectedWidth] && this.getNextCheckedBlock(connectedHeight, connectedWidth, isHorizontalMoving, nextBlock)) {
      if (isHorizontalMoving) {
        if (isPositiveIncrement) {
          connectedWidth++;
      ...
      if (this.edgeOfLargeSquare(connectedHeight, connectedWidth, nextBlock, isHorizontalMoving)) {
        this.squareElements[connectedHeight][connectedWidth].parentClick = false;
      }
    }
    if (isHorizontalMoving) {
      if (isPositiveIncrement) {
   ...
   */
  edgeOfLargeSquare(elementSquareHeight: number, elementSquareWidth: number, nextBlock: number, isHorizontalMoving: boolean): boolean {
    const endOfMadeSquare: boolean = isHorizontalMoving ? this.squareElements[elementSquareHeight][nextBlock] && this.squareElements[elementSquareHeight][nextBlock].parentClick :
      this.squareElements[nextBlock] && this.squareElements[nextBlock][elementSquareWidth] && this.squareElements[nextBlock][elementSquareWidth].parentClick;
    ...
    return elementSquareHeight <= 0 || elementSquareHeight >= LargeSquareComponent.TOTAL_HEIGHT - 1 ||
      elementSquareWidth <= 0 || elementSquareWidth >= LargeSquareComponent.TOTAL_WIDTH - 1;

  }

  getNextCheckedBlock(elementSquareHeight: number, elementSquareWidth: number, isHorizontalMoving: boolean, nextBlock: number) {
    if (nextBlock == null) {
      return true;
    }
    return isHorizontalMoving ? this.squareElements[elementSquareHeight][nextBlock] && this.squareElements[elementSquareHeight][nextBlock].parentClick :
      this.squareElements[nextBlock] && this.squareElements[nextBlock][elementSquareWidth] && this.squareElements[nextBlock][elementSquareWidth].parentClick;
  }
  ...
  displayClickedFromParent(elementClicked: SquareElement) {
    ...
    let nextUpperLeftPosition: elementPlacementInterface = { height: elementClicked.heightPlaced, width: elementClicked.widthPlaced };
    for (let index = 1; index < LargeSquareComponent.TOTAL_HEIGHT + 1; index++) {
      setTimeout(() => {
        if (index === 1) {
          this.squareElements[elementClicked.heightPlaced][elementClicked.widthPlaced].clicked = false;
        }
